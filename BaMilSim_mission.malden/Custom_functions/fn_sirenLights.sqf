...
_vehicleCheck = [_vehType] call BaMilSIm_fnc_sirenLightsConfig;
if(isNil "_vehicleCheck") exitWith {};
if(count _vehicleCheck == 0) exitWith {};
if(count _vehicleCheck == 2) exitWith {};
...
if(_vehicle getVariable["lights",false]) then {
	...
	[_vehicle,_lightpos,_lightcolor] remoteexec["BaMilSIm_fnc_sirenLightsEx",-2];
};